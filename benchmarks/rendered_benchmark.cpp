...
int renderedBenchmark() {
    ...
    auto disk = QSharedPointer<FileDiskCache>::create(directory.path(), 16 * 1024 * 1024, 4 * 1024 * 1024);
    RenderedDiskCache cache(disk);
    RenderOptions options;
    options.physicalTargetSize = QSize(512, 512);
    options.processors = {{"qt-smooth-scale", 1, "2048-to-512"}};
    auto key = *KeyBuilder().render(*KeyBuilder().network(QUrl("https://benchmark/image")).value, options).value;
    ...
    auto resized = original.scaled(options.physicalTargetSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    ...
    for (bool fromDisk : {false, true}) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            auto image = fromDisk ? cache.get(key).value.value_or(QImage())
                                  : original.scaled(options.physicalTargetSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
            if (image.isNull())
                return 2;
            times.push_back(std::chrono::duration<double, std::micro>(std::chrono::steady_clock::now() - before).count());
        }
        std::sort(times.begin(), times.end());
        std::cout << (fromDisk ? "rendered_png_read" : "qt_smooth_scale") << ',' << times[50] << ',' << times[94] << '\n';
    }
    return 0;
}
