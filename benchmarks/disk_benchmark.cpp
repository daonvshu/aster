...
int diskBenchmark()
{
    ...
    for (int count : {10000, 100000})
    {
        ...
        const auto construction =
            std::chrono::duration<double, std::micro>(Timer::now() - start).count();
        ...
        for (int i = 0; i < 1000; ++i)
        {
            ...
            if (!reader.get(keys[(i * 97) % count]))
                return 3;
            samples.push_back(
                std::chrono::duration<double, std::micro>(Timer::now() - before).count());
        }
        ...
        const auto recovered =
            std::chrono::duration<double, std::micro>(Timer::now() - recovery).count();
