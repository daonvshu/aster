...
int lutBenchmark() {
    ...
    std::array<ImageResampler, 2> resamplers = {ImageResampler({}, ResampleExecution::Automatic, KernelEvaluation::Exact),
        ...
        for (auto algorithm : {ImageScaleAlgorithm::Lanczos3, ImageScaleAlgorithm::Lanczos4}) {
            ...
            for (int round = -1; round < 9; ++round)
                for (int position = 0; position < 2; ++position) {
                    ...
                    QElapsedTimer timer;
                    timer.start();
                    const auto result = resamplers[index].resize(input, {target, QRect(QPoint(), target)}, algorithm, running, &metrics);
            ...
            for (int index = 0; index < 2; ++index) {
                ...
                std::cout << (index ? "lut" : "exact") << ',' << int(algorithm) << ',' << size.width() << 'x' << size.height() << ',' << total[index][4] << ','
...
int resamplerBenchmark() {
    const std::atomic<bool> running{false};
    std::array<ImageResampler, 2> resamplers = {ImageResampler({}, ResampleExecution::Scalar), ImageResampler{}};
        ...
        for (const auto algorithm : {ImageScaleAlgorithm::QtFast, ImageScaleAlgorithm::QtSmooth, ImageScaleAlgorithm::Bilinear, ImageScaleAlgorithm::Bicubic,
            ...
            for (int round = -1; round < 5; ++round)
                for (int position = 0; position < 2; ++position) {
                    ...
                    QElapsedTimer timer;
                    timer.start();
                    const auto result = resamplers[backend].resize(image, {target, QRect(QPoint(), target)}, algorithm, running, &metrics);
            ...
            for (int backend = 0; backend < 2; ++backend) {
                ...
                const char* name = backend == 0 ? "scalar" : ImageResampler::simdAvailable() ? "sse2" : "scalar-fallback";
                std::cout << name << ',' << int(algorithm) << ',' << sourceSize.width() << 'x' << sourceSize.height() << ',' << target.width() << 'x'
                          << target.height() << ',' << ms << ',' << (double(target.width()) * target.height() / (ms * 1000)) << ',' << metrics.workingBytes
                          << std::endl;
            }
        }
    }
