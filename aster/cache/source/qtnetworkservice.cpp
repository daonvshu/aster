...
QtNetworkService::QtNetworkService(int timeoutMs)
    : timeoutMs_(timeoutMs) {
    if (timeoutMs <= 0)
        throw std::invalid_argument("timeout must be positive");
}

Result<NetworkResponse> QtNetworkService::fetch(const QUrl& url, const NetworkFetchOptions& options, const std::atomic<bool>& cancelled) {
    using R = Result<NetworkResponse>;
    if (cancelled.load())
        return R::failure(ImageError::Cancelled);
    if (options.cacheOnly)
        return R::failure(ImageError::CacheMiss);
    if (!url.isValid() || (url.scheme() != "http" && url.scheme() != "https") || options.maxBytes <= 0)
