...
{
    return p != CacheReadPolicy::NoCache && p != CacheReadPolicy::BypassDisk &&
           p != CacheReadPolicy::BypassMemory;
}
...
QMap<QByteArray, QByteArray> directives(const QByteArray& value)
{
    ...
    for (auto part : value.toLower().split(','))
    {
        ...
        result.insert((equal < 0 ? part : part.left(equal)).trimmed(),
                      equal < 0 ? QByteArray() : part.mid(equal + 1).trimmed().replace("\"", ""));
    }
    return result;
}
...
SourcePayload payload(const QByteArray& bytes, HttpHeaders headers, const QDateTime& now)
{
    ...
    result.contentType =
        result.headers.value("content-type").split(';').value(0).trimmed().toLower();
    ...
    if (ok && maxAge >= 0)
    {
        ...
        const auto date = QDateTime::fromString(QString::fromLatin1(result.headers.value("date")),
                                                Qt::RFC2822Date);
    ...
    else
    {
        const auto expires = QDateTime::fromString(
            QString::fromLatin1(result.headers.value("expires")), Qt::RFC2822Date);
        if (expires.isValid())
...
Result<SourcePayload> deserialize(const DiskEntry& entry, const QDateTime& now)
{
    ...
    for (auto it = object.begin(); it != object.end(); ++it)
        headers.insert(it.key().toLatin1(),
...
Result<SourceKey> CachedSourceLoader::key(const ImageSource& source) const
{
    ...
    if (source.kind == ImageSource::Kind::Local)
        return builder.localFile(source.url.toLocalFile(), source.context,
                                 source.strictLocalIdentity);
    if (source.kind == ImageSource::Kind::Resource)
    {
        if (source.url.scheme() != "qrc" || !source.url.authority().isEmpty() ||
            source.url.hasQuery() || source.url.hasFragment())
    ...
    stream << QByteArray("aster/data/v1") << c.nameSpace << c.tenant << c.authScope
           << c.contentVariant << c.revision << source.contentType
           << QCryptographicHash::hash(source.data, QCryptographicHash::Sha256);
    return Result<SourceKey>::success({QCryptographicHash::hash(bytes, QCryptographicHash::Sha256),
                                       KeyBuilder::namespaceDigest(c.nameSpace)});
}

Result<SourcePayload> CachedSourceLoader::load(const ImageSource& source, const SourceKey& expected,
                                               const SourceLoadOptions& requested,
                                               const std::atomic<bool>& cancelled)
{
        ...
        else
        {
            ...
            while (!file.atEnd())
            {
                if (cancelled.load())
                    return R::failure(ImageError::Cancelled);
                auto chunk =
                    file.read(std::min<qint64>(65536, options.maxBytes + 1 - value.bytes.size()));
            ...
            if (value.contentType.isEmpty())
                value.contentType =
                    QMimeDatabase().mimeTypeForFile(file.fileName()).name().toLatin1();
            result = R::success(std::move(value), resource ? CacheResultSource::Resource
                                                           : CacheResultSource::LocalFile);
        }
    }
    ...
    if (result && enabled && memoryWrite(options.cache.write) &&
        !config_.excludedMimeTypes.contains(result.value->contentType))
        encoded_->put(expected, *result.value);
    return result;
}

Result<SourcePayload> CachedSourceLoader::network(const ImageSource& source, const SourceKey& key,
                                                  const SourceLoadOptions& options,
                                                  const std::atomic<bool>& cancelled)
{
    using R = Result<SourcePayload>;
    ...
    fetch.reload = options.cache.read == CacheReadPolicy::BypassMemory ||
                   options.cache.read == CacheReadPolicy::NoCache;
    ...
    else if (disk_ && options.enableSourceDisk && diskWrite(options.cache.write))
    {
        try
        {
            ...
            entry.metadata.insert("namespaceDigest",
                                  QString::fromLatin1(key.namespaceDigest.toHex()));
