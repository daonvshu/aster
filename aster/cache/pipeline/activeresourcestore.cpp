...
ImageHandle ActiveResourceStore::find(const RenderKey& key) {
    ...
    auto handle = it == state_->entries.cend() ? ImageHandle{} : ImageHandle(it->image.toStrongRef());
    if (handle)
        ++state_->stats.hits;
    else
        ++state_->stats.misses;
...
ImageHandle ActiveResourceStore::acquire(const RenderKey& key, const QImage& image) {
    ...
    auto value = QSharedPointer<const QImage>(new QImage(image), [weak = QWeakPointer<State>(state), key, generation](const QImage* ptr) {
        delete ptr;
