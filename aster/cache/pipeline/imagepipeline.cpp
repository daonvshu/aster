...
struct RenderTaskKey
{
    ...
    {
        return render == other.render && read == other.read && write == other.write &&
...
{
    return p == CacheReadPolicy::Default || p == CacheReadPolicy::BypassDisk ||
           p == CacheReadPolicy::CacheOnly;
}
...
struct ImagePipeline::State
{
    ...
    void event(PipelineEvent::Kind kind, const QByteArray& digest,
        ...
        [state, key, request](auto done) -> CancelAction
        {
            ...
            *sourceSubscription = state->sources.subscribe(
                ...
                [state, key, request, done, cancelled](Result<QByteArray> bytes)
                {
                    ...
                    {
                        result =
                            ImageResult::failure(ImageError::ProcessingError, "Renderer threw");
                    }
...
{
    return {state_->sources.count() + state_->payloads.count(),
...
Subscription ImagePipeline::request(const SourceRequest& input, Completion callback)
{
    ...
    request.load.enableSourceDisk =
        input.load.enableSourceDisk && (input.diskStrategy == DiskCacheStrategy::SourceOnly ||
    ...
    stream << source.value->digest << qint32(policy.read) << qint32(policy.write)
           << policy.allowStale << request.load.maxBytes << request.load.enableSourceDisk;
            ...
            *subscription = state->payloads.subscribe(
                ...
                [state, pool, request, sourceKey](auto completed) -> CancelAction
                {
                    ...
                    pool->start(QRunnable::create(
                        [state, loader, request, sourceKey, cancelled, completed]
                        {
                            Result<SourcePayload> result;
                            try
                            {
                                result = loader->load(request.source, sourceKey, request.load,
                                                      *cancelled);
                            }
                            catch (...)
                            {
                                result = Result<SourcePayload>::failure(ImageError::IoError,
                                                                        "Source loader threw");
                            }
                            if (cancelled->load())
                                result = Result<SourcePayload>::failure(ImageError::Cancelled);
                            state->event(PipelineEvent::Kind::SourceCompleted, sourceKey.digest,
                ...
                [state, request, key, token, done](Result<SourcePayload> payload)
                {
                    try
                    {
                        ...
                        contentKey.digest = QCryptographicHash::hash(
                            key.digest +
                                QCryptographicHash::hash(p.bytes, QCryptographicHash::Sha256),
                            QCryptographicHash::Sha256);

                        const bool renderedDisk =
                            state->resources.renderedDisk &&
                            (request.diskStrategy == DiskCacheStrategy::RenderedOnly ||
                             request.diskStrategy == DiskCacheStrategy::All ||
                             (request.diskStrategy == DiskCacheStrategy::Automatic &&
                              (request.source.kind == ImageSource::Kind::Local ||
                               request.source.kind == ImageSource::Kind::Resource) &&
                        ...
                        auto finish = [&](ImageResult result)
                        {
                            ...
                            if (result && !p.noStore && writeMemory(request.load.cache.write) &&
                                state->resources.active)
                            {
                                auto shared = state->cache->get(contentKey, true);
                                if (shared)
                                    result.value = std::move(shared.value);
                                result.handle =
                                    state->resources.active->acquire(contentKey, *result.value);
                            }
                            done(std::move(result));
                        };
                        ...
                        if (!p.noStore && readMemory(request.load.cache.read))
                        {
                            if (state->resources.active)
                            {
                                ...
                                if (handle)
                                {
                                    auto result = ImageResult::success(
                                        handle.image(), CacheResultSource::ActiveResource);
                                    result.handle = std::move(handle);
                                    finish(std::move(result));
                                    return;
                                }
                            }
                            auto hit = state->cache->get(
                                contentKey, request.load.cache.allowStale && !p.mustRevalidate);
                            if (hit)
                            {
                                finish(std::move(hit));
                                return;
                        ...
                        if (!p.noStore && renderedDisk && read != CacheReadPolicy::NoCache &&
                            read != CacheReadPolicy::BypassDisk &&
                        ...
                        try
                        {
                            ...
                            if (result)
                            {
                                ...
                                if (!p.noStore && renderedDisk &&
                                    (write == CacheWritePolicy::Default ||
                                     write == CacheWritePolicy::DiskOnly))
                    ...
                    {
                        done(ImageResult::failure(ImageError::ProcessingError,
...
bool ImagePipeline::invalidate(const CacheSelector& selector, bool includeSource)
{
    ...
    attempt(
        [&]
        {
            return state_->cache->invalidate(selector);
        });
    if (state_->resources.active)
        attempt(
            [&]
            {
                return state_->resources.active->invalidate(selector);
            });
    if (state_->resources.renderedDisk)
        attempt(
            [&]
            {
                return state_->resources.renderedDisk->invalidate(selector);
            });
    if (includeSource && state_->loader)
        attempt(
            [&]
            {
                return state_->loader->invalidate(selector);
            });
    return success;
}
...
Subscription ImagePipeline::request(const QString& source, const RenderOptions& render,
                                    Completion callback)
{
...
ImageSubscription* ImagePipeline::request(const QString& source, const RenderOptions& render,
                                          QObject* parent)
{
    auto parsed = ImageSource::fromString(source);
    if (!parsed)
