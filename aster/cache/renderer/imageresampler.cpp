...
double kernel(double x, ImageScaleAlgorithm algorithm, bool lut) {
    ...
    if (lut && x < support) {
        ...
        const qint64 value = qint64(table[size_t(index)]) * 65536 + (qint64(table[size_t(index + 1)]) - table[size_t(index)]) * fraction;
...
std::vector<Contribution> contributions(int source, int destination, int offset, int count, ImageScaleAlgorithm algorithm, const std::atomic<bool>& cancelled,
                                        bool lut) {
    ...
    for (int output = 0; output < count; ++output) {
        ...
        for (int index = first; index <= last; ++index) {
            const double weight = kernel((index - centre) * filterScale, algorithm, lut) * filterScale;
...
void convolve(uchar* output, const Contribution& c, PixelAt pixelAt, bool useSimd) {
    ...
    if (useSimd && c.simdSafe) {
        ...
        for (size_t tap = 0; tap < c.weights.size(); ++tap) {
            ...
            const auto product = _mm_unpacklo_epi16(_mm_mullo_epi16(values, weight), _mm_mulhi_epi16(values, weight));
    ...
    for (int channel = 0; channel < 4; ++channel)
        output[channel] = uchar(std::clamp<qint64>((sum[channel] + weightScale / 2) / weightScale, 0, 255));
...
ImageResampler::ImageResampler(ResampleLimits limits, ResampleExecution execution, KernelEvaluation kernel)
    : limits_(limits)
    , execution_(execution)
    , kernel_(kernel) {
    if (limits.maxSide <= 0 || limits.maxSide > 32768 || limits.maxPixels < 0 || limits.maxWorkingBytes < 0)
...
ImageResult ImageResampler::resize(const QImage& source, const RenderGeometry& geometry, ImageScaleAlgorithm algorithm, const std::atomic<bool>& cancelled,
    ...
    if (source.isNull() || full.isEmpty() || region.isEmpty() || !QRect(QPoint(), full).contains(region) || int(algorithm) < 0 ||
    ...
    auto within = [this](QSize size) {
        return size.width() <= limits_.maxSide && size.height() <= limits_.maxSide &&
                (limits_.maxPixels == 0 || qint64(size.width()) * size.height() <= limits_.maxPixels);
    };
    if (!within(source.size()) || !within(region.size()))
        return ImageResult::failure(ImageError::ResourceLimit);
    const bool qt = algorithm == ImageScaleAlgorithm::QtFast || algorithm == ImageScaleAlgorithm::QtSmooth;
    if (qt && !within(full))
        return ImageResult::failure(ImageError::ResourceLimit);
    const qint64 inputBytes = qint64(source.width()) * source.height() * 4;
    const qint64 outputBytes = qint64(region.width()) * region.height() * 4;
    const qint64 intermediateBytes = qt ? qint64(full.width()) * full.height() * 4 : qint64(region.width()) * source.height() * 4;
    ...
    const qint64 tableBytes = qt ? 0
                                 : (qint64(region.width()) + region.height()) * qint64(sizeof(Contribution)) +
                    (qint64(region.width()) * horizontalTaps + qint64(region.height()) * verticalTaps) * 4 + std::max(horizontalTaps, verticalTaps) * 8;
    const qint64 working = qint64(source.sizeInBytes()) + inputBytes + outputBytes + intermediateBytes + tableBytes;
    ...
    try {
        ...
        if (input.size() == full)
            ...
            const auto scaled =
                    input.scaled(full, Qt::IgnoreAspectRatio, algorithm == ImageScaleAlgorithm::QtSmooth ? Qt::SmoothTransformation : Qt::FastTransformation);
            output = scaled.copy(region);
        } else {
            const bool lut = kernel_ == KernelEvaluation::Lut || (kernel_ == KernelEvaluation::Default && ASTER_LANCZOS_USE_LUT);
            QElapsedTimer weightTimer;
            if (metrics)
                weightTimer.start();
            const auto horizontal = contributions(input.width(), full.width(), region.x(), region.width(), algorithm, cancelled, lut);
            const auto vertical = contributions(input.height(), full.height(), region.y(), region.height(), algorithm, cancelled, lut);
            if (metrics)
            ...
            QImage intermediate(region.width(), lastRow - firstRow + 1, QImage::Format_RGBA8888_Premultiplied);
            ...
            for (int y = 0; y < intermediate.height(); ++y) {
                ...
                for (int x = 0; x < region.width(); ++x)
                    convolve(out + x * 4, horizontal[size_t(x)], [row](int index) { return row + index * 4; }, execution_ == ResampleExecution::Automatic);
            }
            for (int y = 0; y < output.height(); ++y) {
                ...
                for (int x = 0; x < output.width(); ++x)
                    convolve(
                            ...
                            [&intermediate, firstRow, x](int index) { return intermediate.constScanLine(index - firstRow) + x * 4; },
