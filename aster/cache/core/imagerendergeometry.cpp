...
{
    for (const auto fit :
         {ImageFit::Fill, ImageFit::Contain, ImageFit::Cover, ImageFit::None, ImageFit::ScaleDown})
        if (fitName(fit) == name)
...
Result<QSize> physicalTargetSize(QSizeF logicalSize, qreal dpr, int bucket)
{
    ...
    auto dimension = [dpr, bucket](qreal logical)
    {
        ...
        if (!std::isfinite(value) || logical <= 0 || value < 1 ||
...
Result<RenderGeometry> renderGeometry(QSize source, QSize target, ImageFit fit)
{
    ...
    else if (fit != ImageFit::None)
    {
        ...
        {
            const double rounded = fit == ImageFit::Cover ? std::ceil(value * factor)
                                                          : std::floor(value * factor + 1e-9);
    ...
    const QPoint offset((scaled.width() - output.width()) / 2,
                        (scaled.height() - output.height()) / 2);
