...
KeyBuilder::KeyBuilder(UrlNormalizer normalizer) : normalizer_(std::move(normalizer))
{
}
...
Result<SourceKey> KeyBuilder::localFile(const QString& path, const KeyContext& c, bool strict) const
{
    ...
    LocalFingerprint f{
        before.canonicalFilePath(), before.size(), before.lastModified().toMSecsSinceEpoch(), {}};
    if (strict)
    {
        QFile file(path);
    ...
    if (!after.exists() || after.canonicalFilePath() != f.canonicalPath ||
        after.size() != f.byteSize || after.lastModified().toMSecsSinceEpoch() != f.modifiedMs)
