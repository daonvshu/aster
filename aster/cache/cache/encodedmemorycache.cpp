...
Result<SourcePayload> EncodedMemoryCache::get(const SourceKey& key, bool allowStale)
{
    ...
    auto it = found.value();
    if (it->value.expiresAt && now >= *it->value.expiresAt &&
        (!allowStale || it->value.mustRevalidate))
    {
        erase(it);
        ++stats_.misses;
        ++stats_.expirations;
...
bool EncodedMemoryCache::put(const SourceKey& key, const SourcePayload& value)
{
    ...
    if (old != index_.end())
        erase(old.value());
    if (key.digest.size() != 32 || value.noStore || value.bytes.isEmpty() ||
        value.bytes.size() > maxEntry_ || cost > stats_.maxBytes || stats_.maxBytes == 0)
    {
        ++stats_.rejections;
        return false;
    }
