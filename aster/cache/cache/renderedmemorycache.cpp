...
bool RenderedMemoryCache::put(const RenderKey& key, const QImage& image, std::optional<QDateTime> expires) {
...
QVector<CacheDebugEntry> RenderedMemoryCache::debugDump(int limit) const {
    ...
    for (const auto& entry : entries_) {
        ...
        result.push_back({entry.key.digest, entry.key.source.digest, entry.key.source.namespaceDigest, entry.meta.byteSize,
