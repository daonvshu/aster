...
FileDiskCache::FileDiskCache(QString root, qint64 budget, qint64 maxEntry,
    ...
    : root_(QDir(root).absoluteFilePath("aster-cache-" + directoryVersion)),
      maxEntry_(std::max<qint64>(0, std::min<qint64>(maxEntry, std::numeric_limits<int>::max() -
                                                                   maxMetadata - headerSize))),
      clock_(std::move(clock)), checkpoint_(std::move(checkpoint))
{
    if (root.isEmpty() || !clock_ ||
        !QRegularExpression("\\A[A-Za-z0-9_-]{1,64}\\z").match(directoryVersion).hasMatch())
...
Result<DiskEntry> FileDiskCache::readLocked(const QByteArray& key, qint64 now, bool track)
{
    ...
    if (header.size() != headerSize || header.left(8) != magic)
        return corrupt();
    const auto metaSize =
        qFromBigEndian<quint32>(reinterpret_cast<const uchar*>(header.constData() + 8));
    const auto dataSize =
        qFromBigEndian<quint32>(reinterpret_cast<const uchar*>(header.constData() + 12));
    if (metaSize > maxMetadata || dataSize > maxEntry_ || headerSize + metaSize + dataSize != size)
        return corrupt();
    const auto body = file.readAll();
    if (body.size() != size - headerSize ||
        QCryptographicHash::hash(body, QCryptographicHash::Sha256) != header.mid(16, 32))
        return corrupt();
    ...
    return Result<DiskEntry>::success({body.mid(int(metaSize)), metadata},
                                      CacheResultSource::RawDisk);
}
...
bool FileDiskCache::trimLocked(qint64 target, const std::atomic<bool>* token)
{
    ...
    std::sort(keys.begin(), keys.end(),
              ...
              {
                  return index_.value(a).accessed < index_.value(b).accessed;
              });
...
bool FileDiskCache::put(const QByteArray& key, const DiskEntry& entry)
{
    ...
    auto checkpoint = [&](WriteStage stage)
    {
        return !checkpoint_ || checkpoint_(stage);
    };
    ...
    try
    {
        if (file.write(bytes.constData(), first) != first || !checkpoint(WriteStage::TenPercent) ||
            file.write(bytes.constData() + first, half - first) != half - first ||
            !checkpoint(WriteStage::Half) ||
            file.write(bytes.constData() + half, bytes.size() - half) != bytes.size() - half ||
            !checkpoint(WriteStage::BeforeCommit))
        {
            file.cancelWriting();
            ++stats_.ioErrors;
            return false;
...
bool FileDiskCache::invalidate(const CacheSelector& selector, const std::atomic<bool>* token)
{
    ...
    for (const auto& key : keys)
    {
        ...
        if ((selector.kind == CacheSelector::Kind::Rendered && !rendered) ||
            (selector.kind == CacheSelector::Kind::Source && rendered))
            continue;

        const SourceKey source{
            QByteArray::fromHex(meta.value("sourceDigest").toString().toLatin1()),
        ...
        const bool legacy = (meta.contains("http") || meta.contains("encoderVersion")) &&
                            (source.digest.size() != 32 || source.namespaceDigest.size() != 32);
        if (legacy || selector.matches(source) ||
            (!selector.sourceDigest.isEmpty() && key == selector.sourceDigest))
...
bool FileDiskCache::flushAccessTimes(const std::atomic<bool>* token)
{
    ...
    for (auto it = index_.begin(); it != index_.end(); ++it)
    {
        ...
        if (!file.open(QIODevice::ReadWrite) ||
            !file.setFileTime(QDateTime::fromMSecsSinceEpoch(it->accessed).toUTC(),
