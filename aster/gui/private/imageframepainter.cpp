...
void ImageFramePainter::draw(QPainter& painter, const QImage& image, QSize target, cache::ImageFit fit) const {
    if (image.isNull())
        return;
    QSizeF size = QSizeF(image.size()) / image.devicePixelRatio();
    const auto exactTarget = cache::physicalTargetSize(contents_.size(), dpr_);
    const bool preview = !exactTarget || target != *exactTarget.value || fit != fit_ || image.devicePixelRatio() != dpr_;
    ...
    if (preview && displayFit != cache::ImageFit::None) {
        ...
        if (displayFit == cache::ImageFit::Fill)
            ...
            qreal scale = displayFit == cache::ImageFit::Cover ? std::max(sx, sy) : std::min(sx, sy);
            if (displayFit == cache::ImageFit::ScaleDown)
                scale = std::min(qreal(1), scale);
            size *= scale;
        }
    }
    const QRectF destination(QPointF(contents_.x() + (contents_.width() - size.width()) / 2, contents_.y() + (contents_.height() - size.height()) / 2), size);
