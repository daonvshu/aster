...
inline void convolveRgbaSse2(uchar* destination, const int* weights, int tapCount, PixelAt&& pixelAt) {
    ...
    for (int channel = 0; channel < 4; ++channel) {
        const qint64 value = (qint64(channels[channel]) + kFixedWeightScale / 2) / kFixedWeightScale;
...
void parallelRows(int rowCount, Function&& function) {
    ...
    for (int worker = 0; worker < workerCount; ++worker) {
        ...
        workers.emplace_back([firstRow, lastRow, &function] { function(firstRow, lastRow); });
    }
...
QImage PicasaResampler::resizeLanczos4Region(const QImage& source, QSize destinationSize, QRect destinationRegion) {
    ...
    const QImage input = source.format() == QImage::Format_RGBA8888 ? source : source.convertToFormat(QImage::Format_RGBA8888);
    if (input.size() == destinationSize)
        return input.copy(destinationRegion);

    const auto horizontal = buildContributions(input.width(), destinationSize.width(), destinationRegion.x(), destinationRegion.width());
    const auto vertical = buildContributions(input.height(), destinationSize.height(), destinationRegion.y(), destinationRegion.height());
    ...
    parallelRows(intermediateHeight, [&](int firstRow, int lastRow) {
...
#if PICASA_RESAMPLER_HAS_SSE2
                convolveRgbaSse2(dst + 4 * x, weights, tapCount, [src, indexes](int tap) { return src + 4 * indexes[tap]; });
#else
    ...
    parallelRows(destinationRegion.height(), [&](int firstRow, int lastRow) {
        for (int y = firstRow; y < lastRow; ++y) {
            ...
            for (int x = 0; x < destinationRegion.width(); ++x) {
#if PICASA_RESAMPLER_HAS_SSE2
                convolveRgbaSse2(dst + 4 * x, weights, tapCount, [&intermediate, indexes, firstSourceRow, x](int tap) {
                    return intermediate.constScanLine(indexes[tap] - firstSourceRow) + 4 * x;
                });
#else
                qint64 a = 0, r = 0, g = 0, b = 0;
                for (int tap = 0; tap < tapCount; ++tap) {
...
QVector<PicasaResampler::Contribution> PicasaResampler::buildContributions(int sourceLength, int destinationLength, int firstDestination, int count) {
