...
ImageResult awaitResult(ImageSubscription* subscription)
{
    ...
    bool received = false;
    ImageResult result;
    QObject::connect(subscription, &ImageSubscription::finished, &loop,
                     [&](ImageResult value)
                     {
                         received = true;
...
void sourceStrings()
{
    ...
    for (const auto& path : {QString("D:/images/a.png"), QString("D:\\images\\a.png"),
                             QString("\\\\server\\share\\a.png"), QString("//server/share/a.png"),
                             QString("images/a.png"), QString("/tmp/a.png")})
    {
        const auto result = ImageSource::fromString(path);
        require(result && result.value->kind == ImageSource::Kind::Local &&
                    result.value->url.isLocalFile(),
                "Local path detection");
    }
    const auto relative = ImageSource::fromString("images/space #50%.png");
    require(relative && relative.value->url.toLocalFile() ==
                            QDir::current().absoluteFilePath("images/space #50%.png"),
    ...
    require(colon && qrc && colon.value->kind == ImageSource::Kind::Resource &&
                colon.value->url == qrc.value->url,
            "Resource alias normalization");

    for (const auto& invalid : {QString(), QString("  "), QString("ftp://host/a.png"),
                                QString("http:///a.png"), QString("qrc://host/a.png"),
    ...
    const auto fileUrl =
        ImageSource::fromString(QUrl::fromLocalFile(path).toString(QUrl::FullyEncoded));
    require(file && fileUrl, "File URL parse failed");
    require(*loader.key(*file.value).value == *loader.key(*fileUrl.value).value,
            "File URL key mismatch");
    const std::atomic<bool> running{false};
    auto payload = loader.load(*file.value, *loader.key(*file.value).value, {}, running);
    require(payload && payload.value->bytes == bytes &&
    ...
    require(!(*resourceKey.value == *loader.key(*file.value).value),
            "File/resource identity collision");
    payload = resources.load(*colon.value, *resourceKey.value, {}, running);
    require(payload && payload.value->bytes == bytes &&
                payload.source == CacheResultSource::Resource,
            "Resource source read failed");
    SourceLoadOptions cachedOnly;
    cachedOnly.cache.read = CacheReadPolicy::CacheOnly;
    require(resources.load(*colon.value, *resourceKey.value, cachedOnly, running).source ==
                CacheResultSource::EncodedMemory,
            "Resource encoded cache miss");
    require(resources.invalidate({resourceKey.value->digest, {}}), "Resource invalidation failed");
    cachedOnly.maxBytes = 1;
    cachedOnly.cache = {};
    require(!resources.load(*colon.value, *resourceKey.value, cachedOnly, running),
            "Resource size limit bypassed");
    require(!resources.key(*ImageSource::fromString(":/aster-test/missing.ppm").value),
            "Missing resource accepted");
}
...
void deliveryAndCancellation()
{
    ...
    QPointer<ImageSubscription> first =
        pipeline.request(":/aster-test/sample.ppm", options(), &owner);
    ...
    QObject::connect(first, &ImageSubscription::finished, &owner,
                     [&](const ImageResult&)
                     {
                         deliveredThread = QThread::currentThread();
                     });
    auto result = awaitResult(first);
    require(result && result.source == CacheResultSource::Resource &&
                result.value->pixelColor(0, 0) == QColor(Qt::red),
            "Resource signal result is invalid");
    ...
    require(result.source == CacheResultSource::RenderedMemory && renders == 1,
            "Explicit source/cache-hit signal lost");
    ...
    int completions = 0;
    QObject::connect(cancelled, &ImageSubscription::finished, &owner,
                     [&](const ImageResult&)
                     {
                         ++completions;
                     });
    cancelled->cancel();
    cancelled->cancel();
    require(awaitResult(cancelled).error == ImageError::Cancelled,
            "Queued success won over cancellation");
    ...
    QObject::connect(
        ...
        [promise, receiver](ImageResult value)
        {
            promise->set_value(bool(value) && QThread::currentThread() == receiver->thread());
        },
        Qt::QueuedConnection);
    ...
    auto legacy = pipeline.request(QString(":/aster-test/sample.ppm"), options(),
                                   ...
                                   {
                                       callback->set_value(std::move(value));
                                   });
    require(callbackFuture.wait_for(std::chrono::seconds(10)) == std::future_status::ready &&
                bool(callbackFuture.get()),
            "Callback API compatibility failed");
...
void destroyedConsumers()
{
    ...
    QPointer<ImageSubscription> abandoned =
        pipeline.request(":/aster-test/sample.ppm", options(), owner);
    int abandonedCalls = 0;
    QObject::connect(abandoned, &ImageSubscription::finished, &observer,
                     [&](const ImageResult&)
                     {
                         ++abandonedCalls;
                     });
    ...
    require(abandoned.isNull() && abandonedCalls == 0 && renders == 1,
            "Destroyed subscriber received a result or broke coalescing");
    ...
    auto disposable =
        std::make_unique<ImagePipeline>(memory, loader,
                                        [](const auto& bytes, const auto&, const auto&)
                                        {
                                            return ImageResult::success(QImage::fromData(bytes));
                                        });
    ...
    require(result || result.error == ImageError::Cancelled,
            "Pipeline destruction lost completion");
}
}
