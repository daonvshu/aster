...
QImage pattern(QSize size) {
    ...
    for (int y = 0; y < size.height(); ++y)
        for (int x = 0; x < size.width(); ++x)
            image.setPixelColor(x, y, QColor((x * 71 + y * 13) % 256, (x * 17 + y * 39) % 256, (x * 47 + y * 23) % 256));
    return image;
}
...
void geometryTests() {
    ...
    require(renderGeometry(QSize(400, 200), QSize(100, 100), ImageFit::Contain).value->scaledSize == QSize(100, 50), "Contain geometry");
    const auto cover = renderGeometry(QSize(400, 200), QSize(100, 100), ImageFit::Cover);
    require(cover.value->scaledSize == QSize(200, 100) && cover.value->region == QRect(50, 0, 100, 100), "Cover geometry");
    require(renderGeometry(QSize(4, 2), QSize(100, 100), ImageFit::Fill).value->scaledSize == QSize(100, 100), "Fill geometry");
    require(renderGeometry(QSize(4, 2), QSize(100, 100), ImageFit::ScaleDown).value->scaledSize == QSize(4, 2), "ScaleDown geometry");
    require(renderGeometry(QSize(400, 200), QSize(1, 1), ImageFit::None).value->scaledSize == QSize(400, 200), "None geometry");
    require(renderGeometry(QSize(1, 16384), QSize(100, 100), ImageFit::Contain).value->scaledSize == QSize(1, 100), "Narrow geometry");
}

void filterTests() {
    const std::atomic<bool> running{false};
    ImageResampler resampler({}, ResampleExecution::Automatic, KernelEvaluation::Exact);
    for (const auto algorithm : {ImageScaleAlgorithm::QtFast, ImageScaleAlgorithm::QtSmooth, ImageScaleAlgorithm::Bilinear, ImageScaleAlgorithm::Bicubic,
        ...
        for (const auto size : {QSize(1, 1), QSize(1, 37), QSize(39, 1), QSize(31, 27)}) {
            ...
            for (const auto target : {QSize(1, 1), QSize(3, 5), QSize(51, 49)}) {
                ...
                require(std::abs(pixel.red() - 80) <= 2 && std::abs(pixel.alpha() - 128) <= 1, "Flat alpha/color preservation");
            }
        }
        QImage image(2, 1, QImage::Format_ARGB32);
        image.setPixelColor(0, 0, QColor(255, 0, 0, 0));
        image.setPixelColor(1, 0, QColor(0, 0, 255, 255));
        const auto result = resampler.resize(image, {QSize(17, 1), QRect(0, 0, 17, 1)}, algorithm, running);
        require(bool(result), "Transparent edge resize");
        for (int x = 0; x < 17; ++x) {
            const auto* p = result.value->constScanLine(0) + x * 4;
            require(p[0] == 0 && p[0] <= p[3] && p[1] <= p[3] && p[2] <= p[3], "Transparent red leaked or premultiplied invariant broken");
        }
    }
    ...
    const int golden[3][18] = {{35, 22, 29, 147, 49, 103, 112, 76, 177, 51, 70, 57, 163, 97, 131, 128, 124, 205},
    ...
    for (int filter = 0; filter < 3; ++filter) {
        ...
        const auto result = resampler.resize(pattern(QSize(5, 3)), {QSize(3, 2), QRect(0, 0, 3, 2)}, algorithm, running);
        require(bool(result), "Golden filter resize");
        for (int y = 0; y < 2; ++y)
            for (int x = 0; x < 3; ++x)
                for (int channel = 0; channel < 3; ++channel)
                    require(std::abs(int(result.value->constScanLine(y)[x * 4 + channel]) - golden[filter][(y * 3 + x) * 3 + channel]) <= 1,
    ...
    for (const auto target : {QSize(5, 3), QSize(23, 17), QSize(1, 1)}) {
        ...
        const auto result = resampler.resize(image, {target, QRect(QPoint(), target)}, ImageScaleAlgorithm::Lanczos4, running);
    ...
    const auto region = resampler.resize(image, {full, tile}, ImageScaleAlgorithm::Lanczos4, running);
    require(region && region.value->convertToFormat(QImage::Format_RGBA8888) == PicasaResampler::resizeLanczos4Region(image, full, tile),
            "Lanczos4 region mismatch");

    const auto qt = resampler.resize(image, {QSize(7, 5), QRect(0, 0, 7, 5)}, ImageScaleAlgorithm::QtSmooth, running);
    require(qt && *qt.value == image.convertToFormat(QImage::Format_RGBA8888_Premultiplied).scaled(7, 5, Qt::IgnoreAspectRatio, Qt::SmoothTransformation),
    ...
    const auto midpoint = resampler.resize(ramp, {QSize(3, 1), QRect(0, 0, 3, 1)}, ImageScaleAlgorithm::Bilinear, running);
    ...
    for (auto algorithm : {ImageScaleAlgorithm::Bicubic, ImageScaleAlgorithm::Lanczos3}) {
        ...
        auto filtered = resampler.resize(impulse, {QSize(17, 1), QRect(0, 0, 17, 1)}, algorithm, running);
        require(filtered && filtered.value->pixelColor(8, 0).red() == 255, "Impulse centre");
        for (int x = 0; x < 8; ++x)
            require(filtered.value->pixel(x, 0) == filtered.value->pixel(16 - x, 0), "Symmetric filter impulse");
    }
    std::atomic<bool> cancelled{true};
    require(resampler.resize(image, {full, tile}, ImageScaleAlgorithm::Lanczos4, cancelled).error == ImageError::Cancelled, "Resampling cancel");
    ResampleLimits limits;
    limits.maxWorkingBytes = 64;
    require(ImageResampler(limits).resize(image, {full, tile}, ImageScaleAlgorithm::Lanczos4, running).error == ImageError::ResourceLimit, "Memory bound");

    limits = {};
    limits.maxPixels = 1;
    require(ImageResampler(limits).resize(image, {full, tile}, ImageScaleAlgorithm::QtSmooth, running).error == ImageError::ResourceLimit,
    ...
    const auto thumbnail = resampler.resize(large, {QSize(1, 1), QRect(0, 0, 1, 1)}, ImageScaleAlgorithm::QtFast, running, &metrics);
    require(thumbnail && thumbnail.value->pixelColor(0, 0) == QColor(Qt::red), "Default resampling accepts large images");
...
void identityTests() {
    ...
    ImagePipeline pipeline(QSharedPointer<RenderedMemoryCache>::create(65536), QSharedPointer<CachedSourceLoader>::create(nullptr),
    ...
    auto fetch = [&] {
        ...
        auto sub = pipeline.request(request, [&](ImageResult result) { promise.set_value(result); });
        require(future.wait_for(std::chrono::seconds(10)) == std::future_status::ready, "Render timeout");
        return future.get();
    };
...
void resamplerTests() {
                ...
                for (bool alpha : {false, true}) {
                    ...
                    for (int y = 0; y < target.height(); ++y)
                        for (int x = 0; x < target.width() * 4; ++x) {
                            const int delta = std::abs(int(a.value->constScanLine(y)[x]) - int(b.value->constScanLine(y)[x]));
                            maximumError = std::max(maximumError, delta);
                            difference += delta;
                            ++channels;
                        }
                }
    std::cout << "LUT max_channel_error=" << maximumError << " mean_error=" << double(difference) / channels << '\n';
    ...
    for (const auto algorithm : {ImageScaleAlgorithm::Bilinear, ImageScaleAlgorithm::Bicubic, ImageScaleAlgorithm::Lanczos3, ImageScaleAlgorithm::Lanczos4})
        for (const auto sourceSize : {QSize(1, 1), QSize(1, 73), QSize(79, 1), QSize(31, 27), QSize(513, 7)})
            for (const auto target : {QSize(1, 1), QSize(3, 5), QSize(47, 39)}) {
                ...
                const QRect region = target.width() > 3 ? QRect(1, 1, target.width() - 2, target.height() - 2) : QRect(QPoint(), target);
                ...
                require(expected && actual && *expected.value == *actual.value, "SIMD/scalar pixel mismatch");
            }
    geometryTests();
    filterTests();
    identityTests();
