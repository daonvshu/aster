...
{
HttpTestPanel::HttpTestPanel(QWidget* parent) : QWidget(parent)
{
    ...
    const QStringList routes = {
        "image",    "delay/3",  "stream",  "status/404", "status/500",
        "redirect", "etag",     "cache",   "no-store",   "wrong-content-type",
        "corrupt",  "truncate", "oversize"};
    const QStringList names = {
        tr("正常图片"),   tr("延迟 3 秒"), tr("分块慢下载"),    "404",      "500",
        tr("302 重定向"), "ETag / 304",    "Cache-Control",     "no-store", tr("错误 Content-Type"),
        tr("损坏数据"),   tr("中途断流"),  tr("超大响应 40MiB")};
    ...
    const auto request = [this, routes](bool allRoutes)
    {
        ...
        for (const auto& route :
             allRoutes ? routes : QStringList{scenario_->currentData().toString()})
    ...
    connect(race, &QPushButton::clicked, this,
            [this]
            {
                const auto base = baseUrl();
...
QString HttpTestPanel::baseUrl()
{
    ...
    if (!url.isValid() || url.host().isEmpty() || !url.userInfo().isEmpty() ||
        (url.scheme() != "http" && url.scheme() != "https") || url.hasQuery() ||
