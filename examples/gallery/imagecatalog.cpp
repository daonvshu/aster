...
void ImageCatalog::loadFolder(const QString& directory, bool recursive) {
    ...
    connect(watcher, &QFutureWatcher<QStringList>::finished, this, [this, watcher, generation] {
        const auto files = watcher->result();
        watcher->deleteLater();
        if (generation == generation_)
            Q_EMIT ready(files);
    });
    watcher->setFuture(QtConcurrent::run([directory = info.absoluteFilePath(), recursive, suffixes, cancelled] {
        QStringList files;
        QDirIterator it(directory, QDir::Files | QDir::Readable, recursive ? QDirIterator::Subdirectories : QDirIterator::NoIteratorFlags);
        ...
        std::sort(files.begin(), files.end(), [&collator](const QString& a, const QString& b) { return collator.compare(a, b) < 0; });
        return files;
    }));
    Q_EMIT started();
}
