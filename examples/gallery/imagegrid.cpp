...
void ImageGrid::updateItems()
{
    ...
    const int totalHeight =
        int(std::min<qint64>(std::numeric_limits<int>::max(), rows * rowHeight + gap));
    {
        const QSignalBlocker block(verticalScrollBar());
    ...
    const int last = int(std::min<qint64>(
        files_.size(), (qint64(offset) + viewport()->height()) / rowHeight * columns + columns));
    ...
    for (int index = first; index < last; ++index)
    {
        if (!cards_.contains(index))
        {
            ...
            const auto name = sourceUrl.scheme().startsWith("http")
                                  ? sourceUrl.path()
                                  : QFileInfo(files_[index]).fileName();
            ...
            connect(box, &gui::ImageBox::stateChanged, label,
                    [label, box](gui::ImageBoxState state)
                    {
                        const QStringList states = {QObject::tr("空"), QObject::tr("加载中"),
                                                    QObject::tr("成功"), QObject::tr("失败")};
                        const QUrl url(box->source());
                        const auto title = url.scheme().startsWith("http")
                                               ? url.path()
                                               : QFileInfo(box->source()).fileName();
                        label->setText(title + " · " + states[int(state)]);
                        if (state == gui::ImageBoxState::Ready)
                            box->parentWidget()->setToolTip(box->source());
                    });
            connect(box, &gui::ImageBox::loadFailed, widget,
                    [widget, box](cache::ImageError error, const QString& message)
                    {
                        widget->setToolTip(
                            box->source() + "\n" +
        ...
        card.widget->setGeometry(gap + (index % columns) * (cellWidth + gap),
                                 gap + (index / columns) * rowHeight - offset, cellWidth,
                                 rowHeight - gap);
        card.widget->show();
    }
}
