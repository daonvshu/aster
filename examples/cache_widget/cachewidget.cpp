...
CacheWidget::CacheWidget(QSharedPointer<aster::cache::ImagePipeline> pipeline,
                         aster::cache::ImageScaleAlgorithm algorithm, QWidget* parent)
    : QWidget(parent), pipeline_(std::move(pipeline)), algorithm_(algorithm)
{
...
void CacheWidget::setSource(const QString& source)
{
    ...
    connect(subscription, &aster::cache::ImageSubscription::finished, this,
            [this, subscription](aster::cache::ImageResult result)
            {
...
void CacheWidget::paintEvent(QPaintEvent*)
{
    ...
    if (!image_.isNull())
        ...
        painter.drawText(rect(), Qt::AlignCenter,
                         QFileInfo(source_).fileName().isEmpty() ? QStringLiteral("Loading...")
                                                                 : QFileInfo(source_).fileName());
}
